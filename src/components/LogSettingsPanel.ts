...
export class LogSettingsPanel {
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" id="auto-export-size" min="50" max="5000" step="50"
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm 
                              focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  이 개수에 도달하면 파일로 저장
              ...
              <div>
                ...
                <input type="number" id="segment-size" min="10" max="1000" step="10"
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm 
                              focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  메모리 세그먼트 단위 크기
            ...
            <div class="space-y-3">
              ...
              <label class="flex items-center space-x-2">
                <input type="checkbox" id="auto-defragment-enabled" 
                       class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  자동 메모리 정리 활성화
                </span>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                <div class="text-xl font-bold text-indigo-600 dark:text-indigo-400" id="stat-allocated-segments">-</div>
                <div class="text-xs text-gray-500 dark:text-gray-400">할당된 세그먼트</div>
              </div>
              
              <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                <div class="text-xl font-bold text-teal-600 dark:text-teal-400" id="stat-memory-efficiency">-</div>
                <div class="text-xs text-gray-500 dark:text-gray-400">메모리 효율성</div>
              </div>
              
              <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                <div class="text-xl font-bold text-cyan-600 dark:text-cyan-400" id="stat-queued-export">-</div>
                <div class="text-xs text-gray-500 dark:text-gray-400">저장 대기</div>
              </div>
              
              <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                <div class="text-xl font-bold text-pink-600 dark:text-pink-400" id="stat-total-segments">-</div>
                <div class="text-xs text-gray-500 dark:text-gray-400">전체 세그먼트</div>
              </div>
            </div>
          </div>
  ...
  private setupEventListeners(panel: HTMLElement): void {
    ...
    // 메모리 정리
    const defragmentBtn = panel.querySelector('#defragment-memory') as HTMLElement;
    defragmentBtn?.addEventListener('click', () => this.defragmentMemory());
  ...
  private loadCurrentSettings(): void {
    ...
    (this.container.querySelector('#segment-size') as HTMLInputElement).value = config.segmentSize.toString();
    (this.container.querySelector('#auto-export-size') as HTMLInputElement).value = config.autoExportSize.toString();
    (this.container.querySelector('#auto-export-enabled') as HTMLInputElement).checked = config.autoExportEnabled;
    (this.container.querySelector('#auto-defragment-enabled') as HTMLInputElement).checked = config.autoDefragmentEnabled;
    (this.container.querySelector('#defragment-interval') as HTMLInputElement).value = (config.defragmentInterval / 1000).toString();
  ...
  private saveSettings(): void {
    try {
      const newConfig: Partial<LogBufferConfig> = {
        ...
        segmentSize: parseInt((this.container.querySelector('#segment-size') as HTMLInputElement).value),
        autoExportSize: parseInt((this.container.querySelector('#auto-export-size') as HTMLInputElement).value),
        autoExportEnabled: (this.container.querySelector('#auto-export-enabled') as HTMLInputElement).checked,
        autoDefragmentEnabled: (this.container.querySelector('#auto-defragment-enabled') as HTMLInputElement).checked,
        defragmentInterval: parseInt((this.container.querySelector('#defragment-interval') as HTMLInputElement).value) * 1000,
  ...
  private updateStats(): void {
    ...
    (this.container.querySelector('#stat-allocated-segments') as HTMLElement).textContent = `${stats.allocatedSegments}/${stats.totalSegments}`;
    (this.container.querySelector('#stat-memory-efficiency') as HTMLElement).textContent = stats.memoryEfficiency;
    (this.container.querySelector('#stat-queued-export') as HTMLElement).textContent = stats.queuedForExport.toString();
    (this.container.querySelector('#stat-total-segments') as HTMLElement).textContent = stats.totalSegments.toString();
  }
  ...
  private defragmentMemory(): void {
    try {
      this.logService.defragment();
      this.showNotification('메모리 정리가 완료되었습니다!', 'success');
      this.updateStats();
    } catch (error) {
      this.showNotification('메모리 정리 중 오류가 발생했습니다.', 'error');
      console.error('Failed to defragment memory:', error);
    }
  }
